...
`EVD-20260818-09ZEF-2` records the initial verification evidence. `EVD-20260818-09ZEF-3` updates that evidence after Python setup was retried: tree/prospective-integration validation now passes with a temporary virtual environment, while worktree validation remains blocked by upstream submodule scanning behavior. Human acceptance is pending after the later core update.
