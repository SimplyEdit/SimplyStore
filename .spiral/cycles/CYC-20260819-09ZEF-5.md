...
remains historical evidence for the runtime integrity slice. The current cycle
is Active for this bounded process correction; correction acceptance is pending.
...
The branch is ready for human evaluation of this correction. Status remains
Active pending explicit acceptance; the integration validator correctly blocks
it until then. A disposable hypothetical Accepted candidate passed the other
integration constraints. After acceptance, record it in a new commit, update
this context/status, and revalidate against the current target before merge.
