...
Active-cycle progress on 2026-09-19: the first baseline slice is implemented in `IMP-20260919-TTZ7C-22` and verified by `EVD-20260919-TTZ7C-23`. `docs/power-loss-baseline.md` maps 21 scenarios; 10 invariant assertions expose current gaps (explicit TODO in baseline mode, failing in strict mode). Existing suites pass 59/59. Findings include unsynced namespace loss, acknowledged short writes, unsafe replay, missing request inputs for a configured worker, and queue-order changes after restart. The design step is now recorded in `DES-20260919-TTZ7C-24` (durable publication and ordered records) and `DES-20260919-TTZ7C-25` (administrative recovery and consistent restore). No runtime fixes or filesystem power-loss guarantee are claimed. The maintainer subsequently requested a design-only revision retaining current store formats without migration. DES-24/25 now remove the v2 envelope, stored sequence/profile fields, and store descriptor. Commands themselves supply all execution inputs beyond the dataset; HTTP request context is removed from the designed worker contract. Acceptance serialization establishes log/execution order without stored sequence fields. Pending work on restart requires administrator assessment because existing records do not prove which protocol wrote them. Recovery audit/backup reports remain external tool outputs, not required store metadata. Historical missing inputs/order and unobservable history loss remain explicit limits. PL15's request-retention assertion is superseded by the corrected input contract; its observed baseline behavior remains evidence. This step is design only. Proposed implementation starts with complete-write loops, file/directory barriers, and error handling, then ownership/ordering and recovery; remaining test rows and the rest of the cycle stay open.
