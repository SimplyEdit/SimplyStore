...
Current active Spiral cycle: `.spiral/cycles/CYC-20260919-TTZ7C-17.md` (`Power-Loss Durability And Administrative Recovery`), committed by the maintainer on 2026-09-19 and opened on `spiral/CYC-20260919-TTZ7C-17-power-loss-recovery` from `master` at `44ea91717a328ad7b5e049c6c0845b5e3bcc1718`. The cycle is Active; runtime work and acceptance evidence remain outstanding. Its source, understanding, and acceptance matrix are `SRC-20260919-TTZ7C-18`, `UND-20260919-TTZ7C-19`, and `REQ-20260919-TTZ7C-20`.

Latest accepted cycle: `.spiral/cycles/CYC-20260917-TTZ7C-4.md` (`Persisted Offset Indexes`), explicitly accepted on 2026-09-18 and merged into `master` at `0444094`. Conversion and command changesets finalize offsets from their serialized bytes through the configured index module, with a default fallback for existing wrappers. The earlier integrity runtime slice was accepted on 2026-08-19; its compliance correction in `DEF-20260916-TTZ7C-1`, verified by `EVD-20260916-TTZ7C-3`, was explicitly accepted on 2026-09-16 and merged at `e59c6f1`.

Active-cycle progress on 2026-09-19: the first baseline slice is implemented in `IMP-20260919-TTZ7C-22` and verified by `EVD-20260919-TTZ7C-23`. `docs/power-loss-baseline.md` maps 21 scenarios; 10 invariant assertions expose current gaps (explicit TODO in baseline mode, failing in strict mode). Existing suites pass 59/59. Findings include unsynced namespace loss, acknowledged short writes, unsafe replay, missing request inputs for a configured worker, and queue-order changes after restart. The design step is now recorded in `DES-20260919-TTZ7C-24` (durable publication and ordered records) and `DES-20260919-TTZ7C-25` (administrative recovery and consistent restore). No runtime fixes or filesystem power-loss guarantee are claimed. The maintainer subsequently requested a design-only revision retaining current store formats without migration. DES-24/25 now remove the v2 envelope, stored sequence/profile fields, and store descriptor. Commands themselves supply all execution inputs beyond the dataset; HTTP request context is removed from the designed worker contract. Command-log position is the sole authoritative order for live execution, reconstruction, restart, and the later-dataset guard; acceptance serialization must enforce it without stored sequence fields. Contrary status/queue order is a bug, not an alternative authority. PL16's old queue-preservation assertion must be corrected to compare execution against the log. A complete log settles order; historical out-of-order changesets can still require diagnosis because they may encode the wrong preceding state. Pending work on restart requires administrator assessment because existing records do not prove which protocol wrote them. Recovery audit/backup reports remain external tool outputs, not required store metadata. Historical missing inputs/order and unobservable history loss remain explicit limits. PL15's request-retention assertion is superseded by the corrected input contract; its observed baseline behavior remains evidence. This step is design only. Proposed implementation starts with complete-write loops, file/directory barriers, and error handling, then ownership/ordering and recovery; remaining test rows and the rest of the cycle stay open.
