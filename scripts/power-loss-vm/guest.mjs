...
if(mode==='unsynced'){
    await fs.writeFile('/store/unsynced','not guaranteed durable');console.log('CUT_UNSYNCED');await new Promise(()=>{
...
if(mode==='done'){
    for(;;){
        const status=await fetch('http://127.0.0.1:3000/command/A').then(r=>r.json());if(status.status==='done'){
            break;
    ...
    for(let i=0;i<4;i++){
        ...
        for(;;){
            const status=await fetch('http://127.0.0.1:3000/command/'+id).then(r=>r.json());if(status.status==='done'){
                break;
            }await new Promise(r=>setTimeout(r,10))
        }
