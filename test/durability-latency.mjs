...
test('measure local startup, acceptance, and completion latency',async t=>{
    ...
    for(let i=0;i<30;i++){
        ...
        for(;;){
            const status=await getCommandStatus(port,id);if(status.status==='done'){
                break;
            }assert.equal(['accepted','active'].includes(status.status),true);await new Promise(r=>setTimeout(r,2))
        }
        completed.push(performance.now()-start)
    }
    const stats=values=>{
        const v=[...values].sort((a,b)=>a-b);return {p50Ms:v[Math.floor(v.length*.5)],p95Ms:v[Math.floor(v.length*.95)],maxMs:v.at(-1)}
    }
