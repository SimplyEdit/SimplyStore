...
test('measure local startup, acceptance, and completion latency', async t => {
    ...
    const boot = performance.now(),
        server = startServer(t, store, { port })
    ...
    for (let i = 0; i < 30; i++) {
        ...
        for (;;) {
            ...
            assert.equal(['accepted', 'active'].includes(status.status), true)
    ...
    const stats = values => {
        ...
        return {
            p50Ms: v[Math.floor(v.length * 0.5)],
    ...
    t.diagnostic(
        JSON.stringify({
            samples: 30,
            startupMs,
            acceptance: stats(accepted),
            completion: stats(completed),
            filesystem: execFileSync('stat', ['-f', '-c', '%T', store.dir], {
                encoding: 'utf8'
            }).trim(),
            node: process.version,
            notes: 'Single localhost client; completion includes a fresh worker and polling; no production performance claim'
        })
    )
})
