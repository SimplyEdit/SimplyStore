...
import {planRecovery,applyRecovery,backupStore,restoreBackup,verifyCandidate} from '../src/admin-recovery.mjs'
...
async function setup(t, entries) {
    ...
    const outputs=await fs.mkdtemp(path.join(os.tmpdir(),'simplystore-admin-output-'))
    t.after(()=>fs.rm(outputs,{recursive:true,force:true}))
    await fs.writeFile(store.commandLog,entries.map(([id])=>JSONTag.stringify({id,name:'addPerson',value:{name:id}})).join('\n')+'\n')
    await fs.writeFile(store.commandStatus,entries.flatMap(([id,status])=>[{command:id,status:'accepted',code:202},...status==='accepted'?[]:[{command:id,status,code:200}]]).map(s=>JSONTag.stringify(s)).join('\n')+'\n')
    return {store,outputs}
}
async function runRecovery(store,outputs,plan) {
    return applyRecovery(plan,{to:path.join(outputs,'recovered'),auditDir:path.join(outputs,'audit'),approveRerun:plan.rerun,operator:'test admin',reason:'Verified complete log and command inputs; side effects assessed'})
}
...
test('new later file makes an approved plan stale before executing a handler',async t=>{
    ...
    const plan=await planRecovery(store,{quiescent:true})
    await fs.writeFile(path.join(store.dir,'data.B.jsontag'),'')
    await assert.rejects(runRecovery(store,outputs,plan),/Stale recovery plan/)
...
test('changed code and missing explicit approval reject apply',async t=>{
    ...
    const plan=await planRecovery(store,{quiescent:true})
    await assert.rejects(applyRecovery(plan,{to:path.join(outputs,'x'),auditDir:path.join(outputs,'y'),operator:'admin',reason:'test'}),/approval/)
...
test('backup and restore validate saved bytes without executing commands',async t=>{
    ...
    const result=await runRecovery(store,outputs,await planRecovery(store,{quiescent:true}))
    const backup=path.join(outputs,'backup')
    await backupStore(result.config,{to:backup,quiescent:true})
    const restored=await restoreBackup(backup,{to:path.join(outputs,'restored'),auditDir:path.join(outputs,'restore-audit'),source:store,sourceQuiescent:true})
    assert.deepEqual((await inspectStore(restored.config)).data.persons.map(p=>p.name),['A'])
    assert.deepEqual(restored.missingFromBackup,[])
    const saved=JSON.parse(await fs.readFile(path.join(backup,'complete.json'),'utf8'))
    await fs.appendFile(saved.config.commandLog,' ')
    await assert.rejects(restoreBackup(backup,{to:path.join(outputs,'bad'),auditDir:path.join(outputs,'bad-audit')}),/Backup contents/)
})
...
test('read-only inspection never imports handlers and rejects conflicting done order',async t=>{
    ...
    await fs.writeFile(store.commandsFile,"throw new Error('must not be imported')")
    const before=await fs.readFile(store.commandStatus)
    const plan=await planRecovery(store,{quiescent:true})
    assert.equal(plan.actionable,true);assert.deepEqual(await fs.readFile(store.commandStatus),before)
    await fs.appendFile(store.commandStatus,[{command:'B',status:'done'},{command:'A',status:'done'}].map(JSON.stringify).join('\n')+'\n')
    assert.ok((await planRecovery(store,{quiescent:true})).blocks.some(s=>s.includes('differs from command-log order')))
})
test('historical status order never makes a later log dataset eligible for bypass',async t=>{
    const {store}=await setup(t,[['A','accepted'],['B','accepted'],['C','accepted']])
    await fs.writeFile(store.commandStatus,[{command:'C',status:'accepted'},{command:'B',status:'accepted'},{command:'A',status:'accepted'}].map(JSON.stringify).join('\n')+'\n')
    await fs.writeFile(path.join(store.dir,'data.C.jsontag'),'')
    const plan=await planRecovery(store,{quiescent:true})
    assert.ok(plan.blocks.some(s=>s.includes('B: later accepted datasets exist: C')))
})
test('interrupted recovery preserves completed attempts and blocks an unfinished candidate',async t=>{
    const {store,outputs}=await setup(t,[['A','accepted'],['B','accepted']])
    const witness=path.join(outputs,'effects')
    await fs.writeFile(store.commandsFile,`import fs from 'node:fs';export default {addPerson(data,c){fs.appendFileSync(${JSON.stringify(witness)},c.id+'\\n');if(c.id==='B')throw new Error('external effect already happened');data.persons.push(c.value)}}`)
    const plan=await planRecovery(store,{quiescent:true})
    await assert.rejects(runRecovery(store,outputs,plan),/uncertain or failed/)
    const authorization=JSON.parse((await fs.readFile(path.join(outputs,'audit','attempts.jsonl'),'utf8')).split('\n')[0])
    const partial=await inspectStore(authorization.config)
    assert.equal(partial.ready,false);assert.deepEqual(partial.committed,['A'])
    assert.equal(partial.commands.find(c=>c.id==='B').status,'active')
    assert.equal(await fs.readFile(witness,'utf8'),'A\nB\n')
    await assert.rejects(acquireOwnership([path.dirname(authorization.config.datafile)]),/locked/)
    await assert.rejects(fs.access(path.join(outputs,'audit','complete.json')),/ENOENT/)
    const preview=await planRecovery({...authorization.config,commandsFile:store.commandsFile,indexFile:store.indexFile},{quiescent:true})
    assert.deepEqual(preview.rerun,['B'],'completed A is reconstructed, never selected for re-execution')
})
test('retained digest mismatch stops recovery before replacing integrity or appending done',async t=>{
    ...
    await appendIntegrityRecord(integrity,store.datafile,await fs.readFile(store.datafile))
    await appendIntegrityRecord(integrity,path.join(store.dir,'data.A.jsontag'),Buffer.from('different historical bytes'))
    store.integrityFile=integrity
    const plan=await planRecovery(store,{quiescent:true})
    await assert.rejects(runRecovery(store,outputs,plan),/Integrity mismatch/)
    const authorization=JSON.parse((await fs.readFile(path.join(outputs,'audit','attempts.jsonl'),'utf8')).split('\n')[0])
    assert.deepEqual(await fs.readFile(authorization.config.integrityFile),await fs.readFile(integrity))
    const statuses=(await fs.readFile(authorization.config.commandStatus,'utf8')).trim().split('\n').map(JSONTag.parse)
...
test('malformed tails and overlapping paths cannot be accepted as valid stores',async t=>{
    ...
    assert.equal((await planRecovery(store,{quiescent:true})).actionable,false)
    assert.ok((await inspectStore({...store,commandStatus:store.commandLog})).errors.some(e=>e.includes('paths overlap')))
})
...
test('approved recovery resumes from completed candidate prefix without repeating external effects',async t=>{
    ...
    await fs.writeFile(store.commandsFile,`import fs from 'node:fs';export default {addPerson(data,c){
    ...
    await assert.rejects(runRecovery(store,outputs,await planRecovery(store,{quiescent:true})),/uncertain or failed/)
    const authorization=JSON.parse((await fs.readFile(path.join(outputs,'audit','attempts.jsonl'),'utf8')).split('\n')[0])
    const partial={...authorization.config,commandsFile:store.commandsFile,indexFile:store.indexFile}
    ...
    const release=await releaseOfflineLocks(partial,{operator:'test admin',reason:'Inspected retained recovery audit; idempotent B can retry',confirmedStopped:true})
    await fs.writeFile(path.join(outputs,'unlock-evidence.json'),JSON.stringify(release))
    await release.finish()
    const retry=path.join(outputs,'retry');await fs.mkdir(retry)
    const result=await runRecovery(partial,retry,plan)
    assert.deepEqual((await inspectStore(result.config)).data.persons.map(p=>p.name),['A','B'])
    assert.equal(await fs.readFile(witness,'utf8'),'A\nB\n')
})
test('finishing an interrupted completion report reconstructs data without executing again',async t=>{
    ...
    // Fixture models the boundary after done, before completion report/lock release.
    await fs.unlink(path.join(audit,'complete.json'))
    await acquireOwnership([path.dirname(result.config.datafile)],{purpose:'recovery',auditDir:audit})
    const {finishRecovery,releaseOfflineLocks}=await import('../src/admin-recovery.mjs')
    await assert.rejects(releaseOfflineLocks(result.config,{operator:'admin',reason:'test',confirmedStopped:true}),/requires finish/)
    const before=await fs.readFile(result.config.commandStatus)
    const finished=await finishRecovery(audit,{operator:'admin',reason:'All authorized commands already committed',confirmedStopped:true})
    assert.deepEqual(await fs.readFile(result.config.commandStatus),before)
    assert.deepEqual(finished.committed,['A'])
    const owner=await acquireOwnership([path.dirname(result.config.datafile)]);await owner.release()
})
test('multi-directory integrity paths and file permissions survive backup and restore',async t=>{
    const {store,outputs}=await setup(t,[])
    const logdir=path.join(store.dir,'logs');await fs.mkdir(logdir)
    store.commandLog=path.join(logdir,'commands.jsontag');store.commandStatus=path.join(logdir,'status.jsontag')
    await fs.writeFile(store.commandLog,'');await fs.writeFile(store.commandStatus,'')
    store.integrityFile=path.join(logdir,'integrity.jsontag')
    const {appendIntegrityRecord}=await import('../src/integrity.mjs')
    await appendIntegrityRecord(store.integrityFile,store.datafile,await fs.readFile(store.datafile))
    await fs.chmod(store.datafile,0o600)
    const backup=path.join(outputs,'backup');await backupStore(store,{to:backup,quiescent:true})
    const restored=await restoreBackup(backup,{to:path.join(outputs,'restored'),auditDir:path.join(outputs,'audit')})
    ...
    assert.deepEqual(await fs.readFile(restored.config.integrityFile),await fs.readFile(store.integrityFile))
})

test('backup excludes unrelated secrets while retaining declared custom artifacts',async t=>{
    const {store,outputs}=await setup(t,[])
    await fs.writeFile(path.join(store.dir,'.env'),'private application secret')
    const required=path.join(store.dir,'derived-custom.txt');await fs.writeFile(required,'required output')
    const report=await backupStore({...store,requiredFiles:[required]},{to:path.join(outputs,'backup'),quiescent:true})
    assert.equal(Object.keys(report.files).some(file=>path.basename(file)==='.env'),false)
    assert.equal(await fs.readFile(report.config.requiredFiles[0],'utf8'),'required output')
})

test('a backup remains restorable after transfer to a different directory',async t=>{
    const {store,outputs}=await setup(t,[])
    const old=path.join(outputs,'old-backup');await backupStore(store,{to:old,quiescent:true})
    const moved=path.join(outputs,'transferred-backup');await fs.rename(old,moved)
    const result=await restoreBackup(moved,{to:path.join(outputs,'restored'),auditDir:path.join(outputs,'restore-audit')})
...
test('verified A prefix permits only missing B, waiting C and missing D in log order',async t=>{
    ...
    for(const id of ['B','C','D']){
        await fs.unlink(path.join(path.dirname(source.datafile),`data.${id}.jsontag`))
    }
