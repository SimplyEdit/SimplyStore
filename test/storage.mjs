...
test('complete-write loop preserves bytes through repeated short writes and rejects zero progress', async () => {
    const bytes = Buffer.from('café 漢字 😀'),
        written = []
    await writeAll(
        {
            async write(buffer, offset, length) {
                const n = Math.min(3, length)
                written.push(buffer.subarray(offset, offset + n))
...
test('serialized writer preserves order and stops after an uncertain operation', async () => {
    ...
    await assert.rejects(second, /uncertain/)
    assert.deepEqual(seen, ['A'])
})
test('durable publication replaces entire contents and concurrent append retains each record once', async t => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'simplystore-storage-'))
    t.after(() => fs.rm(dir, { recursive: true, force: true }))
    const file = path.join(dir, 'data')
    publishFileSync(file, 'old')
    await publishFile(file, 'new café')
    assert.equal(await fs.readFile(file, 'utf8'), 'new café')
    await publishFile(file, '')
    assert.equal((await fs.stat(file)).size, 0)
    await Promise.all(
        Array.from({ length: 40 }, (_, i) => appendRecord(file, String(i)))
    )
    assert.equal(
    ...
    await assert.rejects(
        publishFile(path.join(dir, 'missing', 'x'), 'x'),
        error => error.code === 'ENOENT' && error.storageFailure
    )
