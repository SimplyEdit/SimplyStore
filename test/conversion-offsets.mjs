...
const converter = fileURLToPath(
	new URL('../scripts/convert.mjs', import.meta.url)
)
...
async function fixture(t) {
	...
	await fs.writeFile(
		input,
		'{"items":[<object id="/first">{"name":"first"},<object id="/second">{"name":"a long original name"}]}'
	)
	return { dir, input, output }
}

async function convert({ input, output }, indexFile) {
	const args = [converter, input, output]
	if (indexFile) {
		args.push(indexFile)
	}
	await run(process.execPath, args, { timeout: 10000 })
}

async function verifyOffsets({ dir, output }) {
	const bytes = await fs.readFile(output)
	const offsets = JSON.parse(
		await fs.readFile(path.join(dir, 'index.offset.json'), 'utf8')
	)
	const payloads = []
	let cursor = 0
	// Derive byte ranges directly from disk framing, independently of
	// Parser.meta.
	while (cursor < bytes.length) {
		const match = /^\((\d+)\)/.exec(
			bytes.subarray(cursor, cursor + 32).toString('ascii')
		)
		...
		assert.deepEqual(
			offsets[payloads.length],
			[start, end],
			`record ${payloads.length} byte range`
		)
		payloads.push(
			bytes.subarray(...offsets[payloads.length]).toString('utf8')
		)
		cursor = end
...
test('default conversion offsets match every final record', async t => {
	...
	const ids = JSON.parse(
		await fs.readFile(path.join(files.dir, 'index.id.json'), 'utf8')
	)
...
test('conversion finalizes offsets after custom indexing changes record sizes and adds records', async t => {
	...
	await fs.writeFile(
		indexFile,
		`import index from ${JSON.stringify(baseIndex)}
export default {
	create(data, meta) {
...
`
	...
	assert.equal(
		data.items[0].name,
		'caf\u00e9 \u6f22\u5b57 \ud83d\ude00 with a much longer derived name'
	)
	assert.equal(data.items[0].derived[0].name, 'x')
	assert.equal(
		Object.getOwnPropertyDescriptor(data.items[0], 'derived').enumerable,
		false
	)
...
test('conversion replaces stale offsets even when the custom hook writes no index', async t => {
	...
	await fs.writeFile(
		indexFile,
		'export default Object.freeze({ create(data) { data.items[0].name = "changed" } })\n'
	)
	await fs.writeFile(
		path.join(files.dir, 'index.offset.json'),
		'{"0":[0,1],"99":[2,3]}'
	)
	await convert(files, indexFile)
	const { payloads, data } = await verifyOffsets(files)
...
`
	...
	assert.deepEqual(
		JSON.parse(
			await fs.readFile(path.join(files.dir, 'finalized.json'), 'utf8')
		),
		{ marker: 'custom receiver', uuid: null }
	)
})
...
test('conversion propagates rejection from custom finalization', async t => {
	...
	await fs.writeFile(
		indexFile,
		'export default {create() {}, async finalize() { throw new Error("custom finalization failed") }}\n'
	)
	await assert.rejects(
		convert(files, indexFile),
		/custom finalization failed/
	)
	await assert.rejects(
		fs.access(path.join(files.dir, 'index.offset.json')),
		/ENOENT/
	)
})
...
test('conversion initializes existing-format logs and optional base integrity without overwriting a store', async t => {
	...
	assert.equal(
		verifyIntegrity(
			...
			integrity,
			files.output,
			await fs.readFile(files.output),
			{ required: true }
		),
		true
	)
	assert.equal(
		await fs.readFile(path.join(files.dir, 'command-log.jsontag'), 'utf8'),
		''
	)
	assert.equal(
		await fs.readFile(
			path.join(files.dir, 'command-status.jsontag'),
			'utf8'
		),
		''
	)
