...
if (configFile) {
	...
	function snapshot(file) {
		if (raw.statSync(file).isDirectory()) {
			return {}
		}
		const bytes = raw.readFileSync(file).toString('base64')
		return { bytes }
	}
	async function fault(op, file, content = '') {
		...
		if (
			rule.after &&
			!raw
				.readFileSync(config.trace, 'utf8')
				.split('\n')
				.filter(Boolean)
				.map(JSON.parse)
				.some(
					event =>
						event.op === rule.after.op &&
						path.basename(event.file) === rule.after.file
				)
		) {
			return false
		}
		...
		if (rule.action === 'pause') {
			...
			throw Object.assign(
				new Error(`Injected ${rule.code || 'EIO'} at ${op}`),
				{ code: rule.code || 'EIO' }
			)
		}
		return false
	...
	fsp.open = async function (file, ...args) {
		...
		for (const method of [
			...
			handle[method] = async (...params) => {
				...
				if (method === 'write') {
					content = String(
						params[0].subarray(params[1], params[1] + params[2])
					)
				}
				...
				let extra = {}
				if (method === 'datasync' || method === 'sync') {
					extra = snapshot(absolute)
				}
				else if (method === 'write') {
					extra = { bytesWritten: result.bytesWritten }
				}
				record(method, absolute, extra)
	...
	fs.write = function (fd, ...args) {
		...
		void fault('write', file)
			.then(short => {
				if (short) {
					// The fixture targets write-file-atomic's Uint8Array
					// overload.
					if (
	...
	fs.rename = function (from, to, callback) {
		const file = path.resolve(String(to))
		void fault('rename', file)
			.then(() =>
				raw.rename(from, to, error => {
					if (!error) {
						record('rename', file, {
