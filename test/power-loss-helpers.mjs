...
export async function fixture(t, options = {}) {
	...
	const audit = await fs.mkdtemp(
		path.join(os.tmpdir(), 'simplystore-observer-')
	)
...
export async function launch(t, store, port, options = {}) {
	...
	const child = spawn(
		...
		[
			'--import',
			fileURLToPath(new URL('./power-loss-preload.mjs', import.meta.url)),
			store.runner
		],
		{
			cwd: store.dir,
			env: {
				...
				SIMPLYSTORE_TEST_OPTIONS: JSON.stringify({
					...
					commandStatus: store.commandStatus,
...
export async function loseUnpublishedName(store, image, original) {
	...
	const publication = trace.findLastIndex(
		e =>
			e.file === original &&
	...
	const durable = trace
		.slice(publication + 1)
		.some(e => e.file === path.dirname(original) && e.op === 'sync')
	if (!durable) {
...
export async function requirement(t, id, actual, expected, message) {
	let todo = 'Known baseline violation; run test:power-loss:strict'
	if (process.env.SIMPLYSTORE_BASELINE_STRICT === '1') {
		todo = false
	}
	await t.test(
		`${id}: ${message}`,
