...
async function ready(t, store, options = {}) {
    const port = await getOpenPort(),
        server = await launch(t, store, port, options)
...
test('PL01-05 durable publication survives modeled loss of unsynced names and restart', async t => {
    const store = await fixture(t),
        server = await ready(t, store)
    await commit(store, server)
    const trace = await rows(store.trace)
    const changeset = path.join(store.dir, 'data.A.jsontag')
    const rename = trace.findIndex(
        e => e.op === 'rename' && e.file === changeset
    )
    assert.ok(rename > 0)
    assert.ok(
        trace
            .slice(0, rename)
            .some(e => e.op === 'sync' && e.file === trace[rename].from)
    )
    const dirSync = trace.findIndex(
        (e, i) => i > rename && e.op === 'sync' && e.file === store.dir
    )
    const doneSync = trace.findIndex(
        (e, i) =>
            i > dirSync &&
            e.op === 'datasync' &&
            e.file === store.commandStatus &&
            Buffer.from(e.bytes, 'base64')
                .toString()
    ...
    assert.deepEqual(
        (await queryPersons(restarted.port)).map(p => p.name),
        ['A']
    )
})
...
test('PL10 an actual short changeset write is completed before acknowledgment', async t => {
    ...
    const trace = await rows(store.trace)
    assert.ok(
        trace.some(e => e.op === 'injected' && e.action === 'short-write')
    )
    assert.ok(
        trace.filter(
            e =>
...
for (const [name, op, file, contains] of [
    ...
    test(`PL07-09/11/18-20 ${name} failure stops mutation without false terminal success`, async t => {
        const store = await fixture(t),
            server = await ready(t, store, {
                fault: { op, file, contains, action: 'error' }
            })
        try {
        ...
        // Failed sync can leave a complete but uncertain done record; never
        // append failed after it.
        const done = status.findIndex(s => s.status === 'done')
        if (done >= 0) {
            assert.equal(
                status.slice(done + 1).some(s => s.status === 'failed'),
                false
            )
        }
        else {
            assert.equal(
                status.some(s => s.status === 'done'),
                false
            )
        }
    })
...
test('PL15 configured workers cannot receive HTTP execution inputs; command values roundtrip', async t => {
    const store = await fixture(t, {
        commandsSource: `export default {addPerson(data,command,request,meta){
...
parentPort.on('message',async task=>{if('request' in task)throw new Error('request leaked to worker');await initialize(task);parentPort.postMessage(await runCommand(task.command,{query:{name:'wrong'}}))})`
    ...
    const input = {
        id: 'A',
        name: 'addPerson',
        value: { name: 'café 漢字 😀', date: new Date('2026-09-19T00:00:00Z') }
    }
    assert.equal(
        (await submit(store, server.port, input, '?name=wrong')).http,
        202
    )
    assert.equal((await completion(store, server.port, 'A')).status, 'done')
    assert.equal(
        JSONTag.stringify((await readCommandLogRecords(store))[0]),
        JSONTag.stringify(input)
    )
...
for (const duplicate of [false, true]) {
    test(`PL16 delayed append cannot be overtaken${duplicate ? ' by duplicate ID' : ''}`, async t => {
        ...
        await eventually(
            async () =>
                (await rows(store.trace)).some(e => e.op === 'injected'),
            'A append gate'
        )
        const b = submit(store, server.port, command(duplicate ? 'A' : 'B'))
        await new Promise(resolve => setTimeout(resolve, 30))
        assert.deepEqual(
            (await readCommandLogRecords(store)).map(c => c.id),
            ['A']
        )
        await fs.writeFile(store.release, '')
        assert.equal((await a).http, 202)
        assert.ok([200, 202].includes((await b).http))
        ...
        assert.deepEqual(
            (await readCommandStatusRecords(store))
                .filter(s => s.status === 'active')
                .map(s => s.command),
            expected
        )
        assert.deepEqual(
            (await queryPersons(server.port)).map(p => p.name),
            expected
        )
    })
}
...
test('PL12/17 startup preserves uncertain active work without repeating external effects', async t => {
    const store = await fixture(t, {
        commandsSource: `import fs from 'node:fs'; export default {addPerson(data,command){fs.appendFileSync(${JSON.stringify('/tmp/placeholder-effect')},'') ;data.persons.push(command.value)}}`
    })
    const witness = path.join(store.audit, 'effects')
    await fs.writeFile(
        store.commandsFile,
        `import fs from 'node:fs'; export default {addPerson(data,command){fs.appendFileSync(${JSON.stringify(witness)},command.id+'\\n');data.persons.push(command.value)}}`
    )
    const server = await ready(t, store, {
        fault: {
            op: 'write',
            file: 'command-status.jsontag',
            contains: '"status":"done"',
            action: 'error'
        }
    })
    await submit(store, server.port, command('A'))
    await waitForExit(server.child).catch(error => {
...
for (const operation of ['sync', 'close:after']) {
    test(`directory ${operation} failure after rename prevents done`, async t => {
        ...
        const server = await ready(t, store, {
            fault: {
                op: operation,
                file: path.basename(store.dir),
                action: 'error',
                after: { op: 'rename', file: 'data.A.jsontag' }
            }
        })
        await submit(store, server.port, command('A'))
        assert.equal((await waitForExit(server.child)).code, 1)
        assert.ok(
            (await rows(store.trace)).some(
                e => e.op === 'injected' && e.file === store.dir
            )
        )
        assert.equal(
            (await readCommandStatusRecords(store)).some(
                c => c.status === 'done'
            ),
            false
        )
    ...
    test(`changeset ${operation} error is not masked or acknowledged`, async t => {
        ...
        const server = await ready(t, store, {
            fault: {
                ...
                file: 'data.A.jsontag.',
                action: 'error',
                code: operation === 'write' ? 'ENOSPC' : 'EIO'
            }
        })
        await submit(store, server.port, command('A'))
        assert.equal((await waitForExit(server.child)).code, 1)
