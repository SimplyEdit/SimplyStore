...
export async function syncAncestors(directory) {
    ...
    for (let current = directory; ; current = path.dirname(current)) {
        ...
        if (parent === current || (await fsp.stat(parent)).dev !== device) {
            break
        }
    }
}
    ...
    try {
        try {
            ...
            if (stat.isSymbolicLink()) {
                throw new Error('Refusing to publish over a symlink')
            }
...
export function appendRecord(file, record) {
    ...
    if (!appenders.has(key)) {
        appenders.set(key, serialWriter())
    }
